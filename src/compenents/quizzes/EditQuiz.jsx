...
export default function EditQuiz() {
    ...
    return (<ThemeContainer>
        <div className="w-[75%] bg-white mx-auto block p-5 rounded-xl">
            <p className="my-5 font-bold">New Question</p>
            <div className="flex my-4">
                <label htmlFor="question" className="block w-[70%]">
                    <input type="text" onChange={val => setQuestion(val.target.value)} id="question" placeholder="Write something or add a link" className="py-2 px-14 rounded shodow-sm bg-gray-200 w-full placeholder-gray-400" />
                </label>
                <div className="block relative w-[30%]">
                    <button className="flex justify-between font-bold bg-color py-2 px-5 mx-3 rounded-xl text-sm" onClick={() => setShow(!show)}><span>{quizType}</span> <FontAwesomeIcon icon={faCaretDown} /></button>
                    {show && <div className="bg-color block rounded-xl p-3 absolute z-10">
                        {quizTypes && quizTypes.map(item => <button onClick={() => setQuizType(item.label)} key={item.id} className={`block text-left font-bold rounded-xl w-full mb-3 p-3 ${quizType == item.label ? 'bg-gradient-to-br from-[#fa9600] to-[#ffe696] hover:bg-gradient-to-br hover:from-amber-700 hover:to-amber-400' : 'bg-white'}`}>
                            {item.label}
                        </button>)}
                    </div>}
                </div>
            </div>
            <p className="my-5 font-bold">Add Answers</p>
            {quizType != 'text' && <div className="flex">
                ...
                <button onClick={handleAddAnswer} className="block mx-3 rounded pointer py-2 px-5 bg-gradient-to-br from-[#fa9600] to-[#ffe696] text-sm hover:bg-gradient-to-br hover:from-amber-700 hover:to-amber-400">Add Text</button>
            </div>}

            {answers && answers.length != 0 && <p className="my-5 font-bold">Answers List</p>}
            ...
            <button onClick={handleAddQuiz} className="block rounded pointer m-2 py-1 px-5 bg-gradient-to-br from-[#fa9600] to-[#ffe696] text-sm hover:bg-gradient-to-br hover:from-amber-700 hover:to-amber-400 mt-10">Add To Quizzes</button>
        </div>
    </ThemeContainer>)
}
