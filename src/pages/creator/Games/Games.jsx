        ...
        <ThemeContainer customeClasses="w-full">
            ...
            <div className="block mx-auto w-[75%]">
                <div className="flex flex-wrap my-5 p-2">
                    <Link to='/add-game' className="block p-4 py-5 my-2 w-[22%] mx-1 text-sm text-center bg-[#1a31d3] rounded-xl relative">
                        <div className="p-4  transition-all hover:scale-105 text-center text-white">
                            <img src="/imgs/addbtn.png" alt="" className="mx-auto" />
                            <p className="">{language && language['add']}</p>
                        </div>
                    </Link>
                    {list && list.map(item => <div key={"cat-" + item.id} className="block p-4 py-5 my-2 w-[22%] mx-1 text-sm text-center bg-[#1a31d3] rounded-xl relative">
                        <button className={`absolute top-0 ${language && language['dir'] == 'ltr' ? 'left-0' : 'right-0'} m-5 cursor-pointer`} onClick={() => setShowModal(item.id)}><FontAwesomeIcon icon={faTrashCan} className="text-xl text-white" /></button>
                        <Link Link to={'/categories/' + item.id} className="p-4  transition-all hover:scale-105 text-center text-white">
