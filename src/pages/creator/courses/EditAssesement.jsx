...
export default function AddAssesement() {
    ...
    const handleAddAssesement = async (e) => {
        ...
        if (assesementType == 'questions') {
            ...
            formData.append("type", assesementType);
        ...
        <ThemeContainer>
            <form method="post" encType="multipart/form-data" onSubmit={handleAddAssesement} className="w-[75%] block mx-auto rounded-xl m-5 bg-white p-5">
                <div className="flex">
                    ...
                    <Link className="m-2 my-3 hover:text-[#4b4b4b]" to={"/courses/" + (data && data.course_id)}>{language && language["course"]}</Link>
                    <FontAwesomeIcon icon={language && language["dir"] == 'ltr' ? faAngleRight : faAngleLeft} className="my-4 m-3 text-color" />
                ...
                <label htmlFor="title" className="block mb-14">
                    ...
                    <input type="text" defaultValue={data && data.title} name="title" id="title" placeholder={language && language["write_here"]} className="py-2 px-14 rounded shodow-sm bg-gray-200 w-full placeholder-gray-400" />
                </label>

                <label htmlFor="description" className="block mb-14">
                    <p className="my-3 font-bold">{language && language["description"]}</p>
                    <textarea defaultValue={data && data.description} name="description" id="description" placeholder={language && language["write_here"]} className="py-2 px-14  rounded shodow-sm bg-gray-200 w-full placeholder-gray-400" rows={3}></textarea>
                </label>
